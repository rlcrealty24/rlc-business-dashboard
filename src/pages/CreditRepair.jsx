...
// ─── Creditor fuzzy matching ──────────────────────────────────────────────────
function normalizeCreditor(name) {
  return (name || '')
    .trim().toLowerCase()
    .replace(/[^a-z0-9 ]/g, ' ')
    .replace(/\b(bank|credit|financial|services?|management|solutions?|recovery|associates?|llc|inc|corp|co|na|usa|auto|card)\b/g, '')
    .replace(/\s+/g, ' ').trim()
}
function creditorsMatch(a, b) {
...
function ScoreTracker({ cid }) {
  ...
  const [inquiries, setInquiries] = useLocalStorage(`credit_${cid}_inquiries`, [])
  ...
  const [importInqs,    setImportInqs]    = useState(true)
  ...
  // ── Diff helpers — fuzzy creditor matching ────────────────────────────────
  const inqKey = i => `${(i.creditor||'').toLowerCase()}|${(i.bureau||'').toLowerCase()}|${i.date||''}`
  const existingNegs     = Array.isArray(negatives)  ? negatives  : []
  const existingCards    = Array.isArray(cards)       ? cards      : []
  const existingInqKeys  = new Set((Array.isArray(inquiries) ? inquiries : []).map(inqKey))

  // Fuzzy neg match: find existing neg that matches a given creditor name
  const findExistingNeg  = (creditor) => existingNegs.find(n => creditorsMatch(n.creditor, creditor))
  const findExistingCard = (name)     => existingCards.find(c => creditorsMatch(c.name, name))

  const previewNegs     = (preview?.negatives || [])
  const previewRevs     = (preview?.accounts  || []).filter(a => a.type === 'Revolving' && a.limit)
  const previewHardInqs = (preview?.inquiries || []).filter(i => i.type === 'Hard' || !i.type)

  const newNegsCount      = previewNegs.filter(n => !findExistingNeg(n.creditor)).length
  const updatedNegsCount  = previewNegs.filter(n =>  findExistingNeg(n.creditor)).length
  const newCardsCount     = previewRevs.filter(c => !findExistingCard(c.name)).length
  const updatedCardsCount = previewRevs.filter(c =>  findExistingCard(c.name)).length
  const newInqsCount      = previewHardInqs.filter(i => !existingInqKeys.has(inqKey(i))).length
  const dupInqsCount      = previewHardInqs.filter(i =>  existingInqKeys.has(inqKey(i))).length
  ...
  function confirmImport() {
    // ── Scores (always append — it's a timeline) ────────────────────────────
    ...
    if (eq || ex || tu) {
      setScores(prev => [...prev, { id: uid(), date: previewScores.date || today(), equifax: eq, experian: ex, transunion: tu }])
    }

    // ── Negatives: update existing (fuzzy match), add truly new ─────────────
    if (importNegs && previewNegs.length) {
      const toAdd = []
      const updatesById = {}
      previewNegs.forEach(n => {
        const existing = findExistingNeg(n.creditor)
        const mapped = {
          creditor:             n.creditor             || '',
        ...
        if (existing) {
          updatesById[existing.id] = mapped
        } else {
          toAdd.push({ ...mapped, id: uid(), addedAt: today(), disputeStatus: 'Not Started', lastDisputeDate: '', notes: 'Imported from credit report' })
        }
      })
      setNegatives(prev => {
        const updated = (Array.isArray(prev) ? prev : []).map(n => {
          const upd = updatesById[n.id]
          return upd ? { ...n, ...upd, notes: (n.notes ? n.notes + '\nUpdated ' + today() : 'Updated ' + today()) } : n
        })
        return [...updated, ...toAdd]
      })
    }

    // ── Cards: update balance/limit on existing (fuzzy), add new ───────────
    if (importAccts && previewRevs.length) {
      const toAdd = []
      const updatesById = {}
      previewRevs.forEach(a => {
        const existing = findExistingCard(a.name)
        if (existing) {
          updatesById[existing.id] = { balance: Number(a.balance) || 0, limit: Number(a.limit) || 0 }
        } else {
          toAdd.push({ id: uid(), name: a.name || 'Unknown', balance: Number(a.balance) || 0, limit: Number(a.limit) || 0, bureau: '' })
        }
      })
      setCards(prev => {
        const updated = (Array.isArray(prev) ? prev : []).map(c => {
          const upd = updatesById[c.id]
          return upd ? { ...c, ...upd } : c
        })
        return [...updated, ...toAdd]
      })
    ...
    // ── Inquiries: skip duplicates, add only new ─────────────────────────────
    if (importInqs && previewHardInqs.length) {
      const toAdd = previewHardInqs
        .filter(i => !existingInqKeys.has(inqKey(i)))
        .map(i => ({ id: uid(), creditor: i.creditor || '', bureau: i.bureau || '', date: i.date || today(), type: i.type || 'Hard', addedAt: today() }))
      if (toAdd.length) setInquiries(prev => [...(Array.isArray(prev) ? prev : []), ...toAdd])
    }
    ...
    <div>
      ...
      {preview && (
        <div style={{
          ...
          {/* What was found — with new/updated counts */}
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            {previewNegs.length > 0 && (
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', background: '#fff', border: '1px solid var(--pink-border)', borderRadius: 'var(--radius)', cursor: 'pointer', fontSize: '0.83rem' }}>
                ...
                <span>
                  {newNegsCount > 0 && <><strong style={{ color: 'var(--green)' }}>+{newNegsCount} new</strong></>}
                  {newNegsCount > 0 && updatedNegsCount > 0 && <span style={{ color: 'var(--text-muted)' }}> · </span>}
                  {updatedNegsCount > 0 && <><strong style={{ color: 'var(--amber)' }}>~{updatedNegsCount} updated</strong></>}
                  <span style={{ color: 'var(--text-muted)', marginLeft: 4 }}>negatives</span>
                </span>
              </label>
            )}
            {previewRevs.length > 0 && (
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', background: '#fff', border: '1px solid var(--pink-border)', borderRadius: 'var(--radius)', cursor: 'pointer', fontSize: '0.83rem' }}>
                <input type="checkbox" checked={importAccts} onChange={e => setImportAccts(e.target.checked)} style={{ accentColor: 'var(--pink)' }} />
                <span>
                  {newCardsCount > 0 && <><strong style={{ color: 'var(--green)' }}>+{newCardsCount} new</strong></>}
                  {newCardsCount > 0 && updatedCardsCount > 0 && <span style={{ color: 'var(--text-muted)' }}> · </span>}
                  {updatedCardsCount > 0 && <><strong style={{ color: 'var(--amber)' }}>~{updatedCardsCount} balance updated</strong></>}
                  <span style={{ color: 'var(--text-muted)', marginLeft: 4 }}>cards</span>
                </span>
              </label>
            )}
            {previewHardInqs.length > 0 && (
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', background: '#fff', border: '1px solid #fcd34d', borderRadius: 'var(--radius)', cursor: 'pointer', fontSize: '0.83rem' }}>
                <input type="checkbox" checked={importInqs} onChange={e => setImportInqs(e.target.checked)} style={{ accentColor: 'var(--amber)' }} />
                <span>
                  {newInqsCount > 0 && <><strong style={{ color: 'var(--green)' }}>+{newInqsCount} new</strong></>}
                  {newInqsCount > 0 && dupInqsCount > 0 && <span style={{ color: 'var(--text-muted)' }}> · </span>}
                  {dupInqsCount > 0 && <><strong style={{ color: 'var(--text-muted)' }}>{dupInqsCount} already recorded</strong></>}
                  <span style={{ color: 'var(--text-muted)', marginLeft: 4 }}>hard inquiries</span>
                </span>
              </label>
            )}
          </div>

          {/* Potentially removed negatives (in file but not on new report) */}
          {importNegs && removedNegs.length > 0 && (
            <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 'var(--radius)', marginBottom: 14, overflow: 'hidden' }}>
              <div style={{ padding: '8px 14px', borderBottom: '1px solid #bbf7d0', fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--green)', fontWeight: 600 }}>
                🎉 {removedNegs.length} item{removedNegs.length !== 1 ? 's' : ''} no longer on this report — may have been removed or resolved
              </div>
              {removedNegs.map((n, i) => (
                <div key={n.id} style={{ padding: '8px 14px', borderBottom: i < removedNegs.length - 1 ? '1px solid #bbf7d0' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontWeight: 600, fontSize: '0.82rem' }}>{n.creditor}</span>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{n.type} · still in profile — review manually</span>
                </div>
              ))}
            </div>
          )}
          ...
          {/* Negative items list */}
          {importNegs && previewNegs.length > 0 && (
            <div style={{ background: '#fff', border: '1px solid var(--pink-border)', borderRadius: 'var(--radius)', marginBottom: 14, overflow: 'hidden' }}>
              ...
              {previewNegs.map((n, i) => {
                const isNew = !findExistingNeg(n.creditor)
                return (
                  <div key={i} style={{ padding: '9px 14px', borderBottom: i < previewNegs.length - 1 ? '1px solid var(--border-light)' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: '0.6rem', fontWeight: 700, padding: '1px 6px', borderRadius: 4, background: isNew ? '#dcfce7' : '#fffbeb', color: isNew ? 'var(--green)' : '#b45309' }}>{isNew ? 'NEW' : 'UPDATE'}</span>
                      <span style={{ fontWeight: 600, fontSize: '0.83rem' }}>{n.creditor}</span>
                      <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{n.type}</span>
                    </div>
          ...
          {/* Hard inquiries list */}
          {importInqs && previewHardInqs.length > 0 && (
            <div style={{ background: '#fff', border: '1px solid #fcd34d', borderRadius: 'var(--radius)', marginBottom: 14, overflow: 'hidden' }}>
              <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border-light)', fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--amber)', fontWeight: 600 }}>
                Hard Inquiries on Report
              </div>
              {previewHardInqs.map((inq, i) => {
                const isNew = !existingInqKeys.has(inqKey(inq))
                return (
                  <div key={i} style={{ padding: '9px 14px', borderBottom: i < previewHardInqs.length - 1 ? '1px solid var(--border-light)' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: '0.6rem', fontWeight: 700, padding: '1px 6px', borderRadius: 4, background: isNew ? '#dcfce7' : '#f3f4f6', color: isNew ? 'var(--green)' : 'var(--text-muted)' }}>{isNew ? 'NEW' : 'DUP'}</span>
                      <span style={{ fontWeight: 600, fontSize: '0.83rem' }}>{inq.creditor}</span>
                      {inq.bureau && <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{inq.bureau}</span>}
                    </div>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', flexShrink: 0 }}>{inq.date || '—'}</span>
                  </div>
                )
              })}
            </div>
                ...
                <div style={{ padding: '8px', display: 'flex', flexDirection: 'column', gap: 8, minHeight: 80 }}>
                  {colItems.map(d => (
                    <div key={d.id} style={{ background: col.bg, border: `1px solid ${col.border}`, borderRadius: 8, padding: '10px 12px' }}>
                      ...
                      {d.method && <div className="text-xs mt-4" style={{ color: col.color, fontWeight:600 }}>{d.method === 'Online' ? '🌐 Online' : '📬 By Letter'}</div>}
...
function NegativeAccounts({ cid }) {
  ...
  function openDisputeModal(a) {
    setDisputeModal(a)
    setDisputeForm({
      creditor:   a.creditor || '',
      account:    a.accountNumber || '',
      bureau:     (a.bureaus||[])[0] || 'Equifax',
      reason:     '',
      status:     'Open',
      method:     'By Letter',
      dateOpened: today(),
  ...
  function saveDispute(e) {
    e.preventDefault()
    setDisputes([...disputes, { ...disputeForm, id: uid() }])
    setAccounts(accounts.map(acc =>
      acc.id === disputeModal.id
        ? { ...acc, disputeStatus: 'Letter Sent', lastDisputeDate: disputeForm.dateOpened }
        : acc
    ))
  ...
  const groups = (() => {
    ...
    return buckets.sort((g1, g2) => (g1[0].creditor || '').localeCompare(g2[0].creditor || ''))
  })()
  ...
  return (
    <div>
      ...
      {dupGroups.length > 0 && (
        <div style={{ background:'#fffbeb', border:'1px solid #fbbf24', borderRadius:'var(--radius)', padding:'10px 14px', marginBottom:16, display:'flex', justifyContent:'space-between', alignItems:'center', gap:12 }}>
          <div>
            <span style={{ fontWeight:700, color:'#b45309', fontSize:'0.83rem' }}>⚠ {dupGroups.length} possible duplicate{dupGroups.length > 1 ? 's' : ''} detected</span>
            <span style={{ fontSize:'0.78rem', color:'var(--text-muted)', marginLeft:8 }}>
              {dupGroups.map(g => g[0].creditor).join(', ')} — review and delete the extra entries below
            </span>
          </div>
        </div>
      )}
                ...
                >
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center' }}>
                      {maxBalance > 0 && <span style={{ fontSize:'0.82rem', fontWeight:700, color:'var(--red)' }}>{formatCurrency(maxBalance)}</span>}
                      ...
                      <div key={a.id} style={{ marginBottom: gi < group.length-1 ? 20 : 0, paddingBottom: gi < group.length-1 ? 20 : 0, borderBottom: gi < group.length-1 ? '1px dashed var(--border-light)' : 'none' }}>
                        ...
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(160px,1fr))', gap:'10px 24px', marginBottom:12 }}>
                          {[
                            ...
                            { label:'Balance',                val: a.balance > 0 ? formatCurrency(a.balance) : null },
      ...
      {/* ── Log Dispute Modal ── */}
      {disputeModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setDisputeModal(null)}>
          <div className="modal">
            <h2>📋 Log Dispute</h2>
            <div style={{ fontSize:'0.82rem', color:'var(--text-muted)', marginBottom:18, padding:'8px 12px', background:'var(--surface-hover)', borderRadius:'var(--radius)', borderLeft:'3px solid var(--pink)' }}>
              <strong style={{ color:'var(--text)' }}>{disputeModal.creditor}</strong>
              {disputeModal.accountNumber && <span> · #{disputeModal.accountNumber}</span>}
              {(disputeModal.bureaus||[]).length > 0 && <span> · {disputeModal.bureaus.join(', ')}</span>}
            </div>
            <form onSubmit={saveDispute}>
              {/* Method toggle */}
              <div className="form-group mb-16">
                <label>How was this disputed?</label>
                <div style={{ display:'flex', gap:10, marginTop:6 }}>
                  {[['📬 By Letter','By Letter'],['🌐 Online','Online']].map(([label, val]) => (
                    <button key={val} type="button"
                      onClick={() => setDisputeForm({...disputeForm, method:val})}
                      style={{
                        flex:1, padding:'10px', borderRadius:'var(--radius)', cursor:'pointer',
                        fontWeight:600, fontSize:'0.84rem',
                        border: disputeForm.method===val ? '2px solid var(--pink)' : '1.5px solid var(--border)',
                        background: disputeForm.method===val ? 'var(--pink-light)' : 'var(--bg)',
                        color: disputeForm.method===val ? 'var(--pink)' : 'var(--text-muted)',
              ...
              <div className="form-row">
                <div className="form-group">
                  <label>Date Disputed</label>
                  <input type="date" value={disputeForm.dateOpened||''} onChange={e => setDisputeForm({...disputeForm, dateOpened:e.target.value})} required />
                </div>
                <div className="form-group">
                  <label>Bureau</label>
                  <select value={disputeForm.bureau||'Equifax'} onChange={e => setDisputeForm({...disputeForm, bureau:e.target.value})}>
                    {BUREAUS.map(b => <option key={b}>{b}</option>)}
                    <option value="All 3 Bureaus">All 3 Bureaus</option>
                  </select>
                </div>
                <div className="form-group">
                  <label>Status</label>
                  <select value={disputeForm.status||'Open'} onChange={e => setDisputeForm({...disputeForm, status:e.target.value})}>
                    {DISPUTE_STATUSES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>
              </div>
              <div className="form-group mb-16">
                ...
                <textarea rows={3} value={disputeForm.reason||''} onChange={e => setDisputeForm({...disputeForm, reason:e.target.value})}
                  placeholder="Not mine, inaccurate amount, paid in full, identity theft, duplicate…" required />
              </div>
              <div className="form-group mb-20">
                <label>Notes</label>
                <textarea rows={2} value={disputeForm.notes||''} onChange={e => setDisputeForm({...disputeForm, notes:e.target.value})}
                  placeholder={disputeForm.method==='By Letter' ? 'Certified mail tracking #, return receipt…' : 'Portal used (AnnualCreditReport, bureau website), confirmation #…'} />
              </div>
              <div className="modal-footer">
                <button type="button" className="btn" onClick={() => setDisputeModal(null)}>Cancel</button>
                <button type="submit" className="btn btn-primary">📋 Save & Add to Disputes Tab</button>
              </div>
            </form>
          </div>
        </div>
...
// ─── Hard Inquiries ───────────────────────────────────────────────────────────
function HardInquiries({ cid }) {
  const [inquiries, setInquiries] = useLocalStorage(`credit_${cid}_inquiries`, [])
  const [adding, setAdding]       = useState(false)
  const [form, setForm]           = useState({ creditor: '', bureau: 'Equifax', date: today(), type: 'Hard' })

  const BUREAUS = ['Equifax', 'Experian', 'TransUnion']

  function save(e) {
    e.preventDefault()
    setInquiries([...(Array.isArray(inquiries) ? inquiries : []), { ...form, id: uid(), addedAt: today() }])
    setForm({ creditor: '', bureau: 'Equifax', date: today(), type: 'Hard' })
    setAdding(false)
  }

  function remove(id) {
    setInquiries((Array.isArray(inquiries) ? inquiries : []).filter(i => i.id !== id))
  }

  // 2-year fall-off: hard inquiries drop off after 24 months
  function fallOffDate(dateStr) {
    if (!dateStr) return null
  ...
  function daysUntilFallOff(dateStr) {
    const fo = fallOffDate(dateStr)
    if (!fo) return null
    const now = new Date()
    now.setHours(0, 0, 0, 0)
    return Math.ceil((fo - now) / 86400000)
  }

  const sorted = [...(Array.isArray(inquiries) ? inquiries : [])].sort((a, b) => (b.date || '').localeCompare(a.date || ''))
  const byBureau = BUREAUS.map(b => ({ bureau: b, items: sorted.filter(i => i.bureau === b) }))
  const hardCount = sorted.filter(i => i.type === 'Hard' || !i.type).length

  const bureauColor = { Equifax: '#e31837', Experian: '#005EB8', TransUnion: '#00A0DF' }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 700 }}>Hard Inquiries</h3>
          <p style={{ margin: '2px 0 0', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
            Hard inquiries stay on your report for 2 years and can lower your score.
          </p>
        </div>
        <button className="btn btn-primary btn-sm" onClick={() => setAdding(true)}>+ Add Inquiry</button>
      </div>

      {/* Summary bar */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
        {BUREAUS.map(b => {
          const cnt = sorted.filter(i => (i.bureau === b) && (i.type === 'Hard' || !i.type)).length
          return (
            <div key={b} style={{ flex: 1, minWidth: 100, padding: '10px 14px', background: '#fff', border: `2px solid ${bureauColor[b]}20`, borderRadius: 'var(--radius)', textAlign: 'center' }}>
              <div style={{ fontSize: '1.4rem', fontWeight: 700, color: bureauColor[b] }}>{cnt}</div>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 2 }}>{b}</div>
            </div>
          )
        })}
        <div style={{ flex: 1, minWidth: 100, padding: '10px 14px', background: '#fff', border: '2px solid var(--border-light)', borderRadius: 'var(--radius)', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--text)' }}>{hardCount}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 2 }}>Total Hard</div>
        </div>
      </div>

      {/* Add form */}
      {adding && (
        <div style={{ background: '#fff', border: '1px solid var(--border-light)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 16 }}>
          <form onSubmit={save}>
            <div className="form-row">
              <div className="form-group" style={{ gridColumn: 'span 2' }}>
                <label>Creditor / Company Name</label>
                <input placeholder="e.g. Capital One" value={form.creditor} onChange={e => setForm({ ...form, creditor: e.target.value })} required />
              </div>
              <div className="form-group">
                <label>Bureau</label>
      ...
      {/* Inquiry list grouped by bureau */}
      {sorted.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
          No inquiries recorded yet. Upload a credit report or add manually.
        </div>
      ) : (
        byBureau.filter(g => g.items.length > 0).map(({ bureau, items: bItems }) => (
          <div key={bureau} style={{ marginBottom: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', background: bureauColor[bureau] }} />
              <span style={{ fontWeight: 600, fontSize: '0.85rem' }}>{bureau}</span>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>({bItems.length} inquir{bItems.length !== 1 ? 'ies' : 'y'})</span>
            </div>
            <div style={{ background: '#fff', border: '1px solid var(--border-light)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
              {bItems.map((inq, idx) => {
                const days = daysUntilFallOff(inq.date)
                const fo   = fallOffDate(inq.date)
                const expired = days !== null && days <= 0
                const soon    = days !== null && days > 0 && days <= 90
                return (
                  <div key={inq.id} style={{ padding: '10px 14px', borderBottom: idx < bItems.length - 1 ? '1px solid var(--border-light)' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, opacity: expired ? 0.5 : 1 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontWeight: 600, fontSize: '0.85rem' }}>{inq.creditor}</span>
                        {(inq.type === 'Soft') && <span className="badge badge-gray" style={{ fontSize: '0.65rem' }}>Soft</span>}
                        {expired && <span className="badge badge-green" style={{ fontSize: '0.65rem' }}>Fallen Off</span>}
                        {soon && !expired && <span className="badge badge-amber" style={{ fontSize: '0.65rem' }}>Falls off soon</span>}
                      </div>
                      <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 2 }}>
                        Pulled: {inq.date ? formatDate(inq.date) : '—'}
                        {fo && !expired && <span style={{ marginLeft: 10 }}>Falls off: {fo.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}{days !== null && ` (${days}d)`}</span>}
                      </div>
                    </div>
                    <button onClick={() => remove(inq.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '1rem', padding: '2px 4px', lineHeight: 1 }} title="Remove">×</button>
                  </div>
                )
              })}
            </div>
