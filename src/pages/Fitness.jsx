...
function Overview({ logs = {}, weekly = [] }) {
  ...
  return (
    <div>
      ...
      <div className="metrics-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: currentWeight ? 12 : 24 }}>
        <div className={`metric-card ${currentWeight ? 'metric-card-blue' : 'metric-card-expense'}`}>
          <div className="metric-label">⚖️ Current Weight</div>
          <div className="metric-value">
            {currentWeight ? <>{currentWeight} <span style={{ fontSize:'1rem' }}>lb</span></> : <span style={{ fontSize:'1.1rem', color:'var(--text-faint)' }}>—</span>}
          </div>
          {recentChange !== null
        ...
        <div className="metric-card metric-card-income">
          <div className="metric-label">📉 Lost So Far</div>
        ...
        <div className="metric-card metric-card-pending">
          ...
          <div className="metric-value">
            {toGoal && parseFloat(toGoal) > 0
              ? <>{toGoal} <span style={{ fontSize:'1rem' }}>lb</span></>
              : toGoal && parseFloat(toGoal) <= 0
...
function DailyLogTab({ logs, setLogs }) {
  ...
  const activeDayIdx = log.workoutDayIdx ?? new Date(selectedDate + 'T00:00:00').getDay()
  ...
  return (
    <div>
      {/* ── Date Navigation Bar ── */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginBottom: 20, padding: '12px 20px', background: 'var(--surface)', border: '1px solid var(--pink-border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow)' }}>
        <button onClick={goBack} style={{ background: 'var(--pink-light)', border: '1.5px solid var(--pink-border)', borderRadius: 8, width: 36, height: 36, cursor: 'pointer', color: 'var(--pink-text)', fontSize: '1.3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', flexShrink: 0, transition: 'background 0.15s' }}>‹</button>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--text)', letterSpacing: '-0.01em' }}>
            {new Date(selectedDate + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
          </div>
          {isToday
            ? <div style={{ fontSize: '0.68rem', color: 'var(--pink-text)', fontWeight: 600, marginTop: 2 }}>Today</div>
            : <button onClick={() => setSelectedDate(today())} style={{ fontSize: '0.68rem', color: 'var(--pink-text)', fontWeight: 600, marginTop: 2, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}>Back to Today</button>
          }
        </div>
        <button onClick={goForward} style={{ background: 'var(--pink-light)', border: '1.5px solid var(--pink-border)', borderRadius: 8, width: 36, height: 36, cursor: 'pointer', color: 'var(--pink-text)', fontSize: '1.3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', flexShrink: 0 }}>›</button>
      </div>
      ...
      <div className="card" style={{ marginBottom: 24, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 20px 12px', background: 'linear-gradient(135deg, var(--pink-light) 0%, #fff8fb 100%)', borderBottom: '1px solid var(--pink-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={goBack} style={{ background: 'none', border: '1px solid var(--pink-border)', borderRadius: 6, width: 28, height: 28, cursor: 'pointer', color: 'var(--pink-text)', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>‹</button>
            <div>
              <h3 style={{ color: 'var(--pink-text)', fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 'bold' }}>
                🍽️ Daily Nutrition — {new Date(selectedDate + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                {isToday && <span style={{ marginLeft: 6, fontSize: '0.65rem', background: 'var(--pink)', color: '#fff', borderRadius: 4, padding: '1px 6px', fontWeight: 600, verticalAlign: 'middle' }}>Today</span>}
              </h3>
            </div>
            <button onClick={goForward} style={{ background: 'none', border: '1px solid var(--pink-border)', borderRadius: 6, width: 28, height: 28, cursor: 'pointer', color: 'var(--pink-text)', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>›</button>
          </div>
      ...
      {/* ── Today's Weight ── */}
      {(() => {
        const currWeight = log.weight ? parseFloat(log.weight) : null
        const sortedDates = Object.keys(logs).filter(d => d < dateKey && logs[d]?.weight).sort()
        ...
        return (
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:'14px 18px', marginBottom:20, display:'flex', alignItems:'center', gap:14, flexWrap:'wrap', boxShadow:'var(--shadow)' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
              <input
                type="number" step="0.1" placeholder="e.g. 146.2"
                value={log.weight || ''}
                onChange={e => updateLog({ weight: e.target.value })}
                style={{ width:100, fontWeight:'bold', fontSize:'1rem', textAlign:'center', padding:'6px 8px' }}
              />
              <span style={{ fontSize:'0.78rem', color:'var(--text-muted)', fontWeight:600 }}>lbs</span>
            </div>
            {diff !== null && (
              <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.8rem', background: parseFloat(diff)<0 ? 'var(--green-bg)' : parseFloat(diff)>0 ? 'var(--red-bg)' : 'var(--border-light)', borderRadius:20, padding:'4px 12px', flexShrink:0 }}>
                <span style={{ fontWeight:700, color: parseFloat(diff)<0 ? 'var(--green)' : parseFloat(diff)>0 ? 'var(--red)' : 'var(--text-muted)' }}>
                  {parseFloat(diff)>0?'+':''}{diff} lbs
                </span>
                <span style={{ color:'var(--text-muted)', fontSize:'0.7rem' }}>vs {new Date(prevDate+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>
              </div>
            )}
            {currWeight && lostTotal !== null && (
              <div style={{ fontSize:'0.75rem', color:'var(--text-muted)', marginLeft:'auto', textAlign:'right', whiteSpace:'nowrap' }}>
                {parseFloat(lostTotal) > 0 && <span style={{ color:'var(--green)', fontWeight:600 }}>−{lostTotal} lbs lost</span>}
                {parseFloat(toGoal) > 0
                  ? <span> · {toGoal} lbs to goal</span>
                  : <span style={{ color:'var(--green)', fontWeight:700 }}> · Goal reached! 🎉</span>}
              </div>
            )}
          </div>
        )
